...
const { CreateProtectedKeyRequest } = require("./create-protected-key-request");
const { CreateKeyRequest } = require("./create-key-request");
const { SignHashRequest } = require("./sign-hash-request");
const { SignHashResponse } = require("./sign-hash-response");
const { CertificateSummary2 } = require("./certificate-summary-2");
...
class AmpliaClient {
     ...
     * @returns {KeyModel} the representation of the created order
     */
    createRSAProtectedKeyAsync(request) {
        return new Promise((resolve, reject) => {
            if (!request) {
                throw new Error("The request was not set");
            }
            ...
            client
                .post(`api/keys/protected`, request)
                .then((response) => {
                    let model = response.getBody();
                    let order = new KeyModel(model);
                    resolve(order);
     ...
     */
	 signHashWithKey(keyId, request) {
        return new Promise((resolve, reject) => {
            ...
            client
                .post(`api/keys/${keyId}/to-sign-hash`, request)
                .then((response) => {
                    let model = response.getBody();
                    let order = new SignHashResponse(model);
                    resolve(order);
                })
                .catch((err) => reject(err));
        });
	...
	issueCertificate(orderId, csr, keyMedia = null, keyId = null) {
		return new Promise((resolve, reject) => {
			let client = this._getRestClient();
            let request = keyId == null ? new IssueCertificateRequest({
				orderId,
                csr,
                keyMedia
            }) : new IssueCertificateRequest({ orderId, keyId });
    ...
    listCertificates(searchParams = null, validOnly = false, subscriptionId = null) {
        return new Promise((resolve, reject) => {
            if (
                searchParams &&
            ...
            let client = this._getRestClient();
            let url =
                AmpliaClient._setPaginatedSearchParams(
                    "api/v2/certificates",
                    searchParams || new PaginatedSearchParams()
                ) + `&validOnly=${validOnly}`
