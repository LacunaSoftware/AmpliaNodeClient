...
class RestClient {
		...
		return new Promise((resolve, reject) => {
			...
			try {
				request.get(requestUrl, params, (error, response) => {
					let err = RestClient._checkResponse(error, response);
					if (err) {
						reject(err);
						return;
					}
				...
				throw new RestUnreachableError('GET', url);
			}
		});
	}
	...
	post(url, data) {
		return new Promise((resolve, reject) => {
			...
			try {
				...
				throw new RestUnreachableError('POST', url);
			}
		});
	}
	...
	del(url) {
		return new Promise((resolve, reject) => {
			...
			try {
				...
				throw new RestUnreachableError('DELETE', url);
			}
		});
	}
	...
	static _checkResponse(error, response) {
		let verb = response.req.method;
		let url = response.req.path;
