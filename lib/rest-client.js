...
const { OrderLocketError } = require('./order-locket-error');
...
class RestClient {
	...
	put(url, data) {
		return new Promise((resolve, reject) => {
			...
			let params = this._getRequestParams(data);
			try {
				request.put(requestUrl, params, (requestError, httpResponse) => {

					if (requestError) {
						reject(new RestUnreachableError(HttpMethods.PUT, url, requestError));
						return;
					}

					let restError = RestClient._checkResponse(HttpMethods.PUT, url, httpResponse);
					if (restError) {
						reject(restError);
						return;
					}
				...
				reject(new RestUnreachableError(HttpMethods.PUT, url, err));
			}
		});
	}
	...
	_getRequestParams(data) {
		...
		if (this._customRequestHeaders) {
			for (let header in this._customRequestHeaders) {
				if (Object.prototype.hasOwnProperty.call(this._customRequestHeaders, header)) {
